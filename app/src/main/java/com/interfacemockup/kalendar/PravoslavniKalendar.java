...
public class PravoslavniKalendar {
    ...
    private int      _broj  = 0;
    private com.interfacemockup.kalendar.pravoslavnekalkulacije.PravoslavneKonstante konstante = new PravoslavneKonstante();
    ...
    @RequiresApi(api = Build.VERSION_CODES.N)
    public int getRedniBrojMeseca(int counter){
        Calendar cal   = GregorianCalendar.getInstance();
        //cal.set(Calendar.YEAR, getGodinu(counter));
        cal.set(Calendar.MONTH, getMesec(counter));
        cal.getTime();
        _broj = cal.get(Calendar.MONTH);
        return _broj;
    }

/*    @RequiresApi(api = Build.VERSION_CODES.N)
    public int getRBMeseca(int counter){
        Calendar cal   = GregorianCalendar.getInstance();
        cal.set(Calendar.MONTH, getMesec(counter));
        cal.getTime();
    ...
    @RequiresApi(api = Build.VERSION_CODES.N)
    public int getMesec(int counter){
    ...
    public int mes(int count){
        int rb;
        Date novi = this.getNovidatum(count);
        Calendar cal = GregorianCalendar.getInstance();
        cal.setTime(novi);
        rb = cal.get(Calendar.MONTH);
        return rb;
    }
    ...
    @RequiresApi(api = Build.VERSION_CODES.N)
    public int get_Mesec(int counter){
        Date datum = getNovidatum(counter);
        String pattern = "d";
        Locale locale = new Locale("sr", "SR");
        SimpleDateFormat danMesecni = new SimpleDateFormat(pattern, locale);
        String dan = danMesecni.format(datum);
